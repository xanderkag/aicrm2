...
async function run() {
  console.log('--- Client Name & AI Migration ---');
  try {
    ...
    for (const p of projects.rows) {
      ...
      try {
        ...
        // Ensure is_ai_enabled also exists (from previous migration)
        await pool.query(`ALTER TABLE "${p.schema_name}"."clients" ADD COLUMN IF NOT EXISTS is_ai_enabled BOOLEAN DEFAULT TRUE`);
        
        console.log(`- SUCCESS: ${p.schema_name}.clients updated with "name" and "is_ai_enabled".`);
      } catch (err) {
